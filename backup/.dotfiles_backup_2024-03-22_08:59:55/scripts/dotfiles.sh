...
main() {
    ...
    declare -A files=(
        ...
        ["$HOME/.config/qtile/rofi/custom_launcher.sh"]="rice"
        ["$HOME/.config/qtile/rofi/sands.jpg"]="rice"
        ["$HOME/.config/qtile/rofi/sands.rasi"]="rice"
 ...
 # Copy files to dotfiles directory
    for file in "${!files[@]}"; do
        if [ -e "$file" ] && [ "$file" != "$DOTFILES_DIR" ] && [ "$(basename "$file")" != ".git" ]; then
            ...
            echo "File not found, attempting to copy dotfiles directory, or .git folder: $file"
        fi
    done
}
